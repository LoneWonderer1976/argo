...
Asks for the account's email and password (and the MFA code if the account has one), then:
  1. caches the tokens in .garmin_tokens/ (git-ignored) so `python -m argo.sync` works locally;
  ...
  3. fetches everything since SCHEME_START locally as well, so `python -m argo.score --print`
     shows the ledger straight away.
...
def main() -> None:
    ...
    print("Garmin Connect login for Argo (the account Thomas's watch syncs to)")
    ...
    from . import sync
    sync.run()
    print("done -- `python -m argo.score --print` shows the ledger")
