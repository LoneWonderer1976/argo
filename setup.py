"""setup.py -- put Argo on GitHub: the repository, the page, the variables, the first run.
...
What it does, in order:
  ...
  2. creates the private repository <you>/argo from this folder and pushes it (or just pushes)
  3. switches on GitHub Pages from main:/docs and records the page's address as the PAGE_URL variable
  4. tells you whether the GARMINTOKENS secret is set -- `python -m argo.login` sets it -- and, if it
    ...
    if git("remote", "get-url", "origin", check=False).returncode != 0:
        if gh("repo", "view", full, check=False).returncode == 0:
            ...
            gh("repo", "create", REPO, "--private", "--source", str(ROOT), "--remote", "origin", "--push",
               "--description", "Pocket money for getting out: Garmin -> points -> pounds")
